        ...
        <div className="min-h-screen relative overflow-hidden flex flex-col justify-between p-12 md:p-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-16 z-10 max-w-7xl w-full mx-auto">
                <div className="flex-1 flex justify-center items-center relative order-1 md:order-2 min-h-[400px] md:min-h-[500px] z-10 overflow-visible">
                    <img src={`${import.meta.env.BASE_URL}fondo-doña-hilda.png`} alt="Fondo" className="md:hidden h-auto absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[48%] z-[5] scale-[1.15]" style={{ maskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)' }} />
                    <img src={`${import.meta.env.BASE_URL}fondo-doña-hilda.png`} alt="Fondo" className="hidden md:block w-[450px] h-auto absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[5]" style={{ maskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)' }} />
                    <img src={`${import.meta.env.BASE_URL}doña-hilda.png`} alt="Hilda Palacios" className="w-[280px] md:w-[400px] h-auto relative z-[15]" style={{ maskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)' }} />
                </div>
